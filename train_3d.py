...
from data_custom import PybulletVisionDataset
import utils
...
def train(conf, train_shape_list, train_data_list, val_data_list, all_train_data_list):
    ...
    # create models
    network = model_def.Network(conf.feat_dim, conf.rv_dim, conf.rv_cnt) # 128, 10, 100
    ...
    if (not conf.resume) and (conf.pretrained_critic_ckpt is not None):
        ...
        for k in pretrained_critic_ckpt:
            if k.startswith('pointnet2.'):
                ...
                cur_param_name = '.'.join(k.split('.')[1:]) # 去掉键名中的特定前缀，只保留后面的部分. k='pointnet2.layer1.weights' -> 'layer1.weights'
    ...
    # load dataset
    train_dataset = PybulletVisionDataset([conf.primact_type], conf.category_types, data_features, conf.buffer_max_num, \
    ...
    val_dataset = PybulletVisionDataset([conf.primact_type], conf.category_types, data_features, conf.buffer_max_num, \
            abs_thres=conf.abs_thres, rel_thres=conf.rel_thres, dp_thres=conf.dp_thres, img_size=conf.img_size, no_true_false_equal=conf.no_true_false_equal)
...
if __name__ == '__main__':
    ...
    # prepare data_dir
    conf.data_dir = conf.data_dir_prefix + '-' + conf.exp_name # ../data/gt_data-exp-model_3d-pushing-None-train_3d
