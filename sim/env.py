...
from utils import link_from_name
# current_dir = os.path.dirname(os.path.abspath(__file__))
# sys.path.append(os.path.join(current_dir, 'pybullet_planning'))
external_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..')) 
sys.path.append(external_dir) 
# from ..pybullet_planning.interfaces.env_manager import connect
from pybullet_planning import connect, load_model, get_max_force
...
class ClutteredPushGrasp:
    ...
    def __init__(self, models: Models, vis=False, num_objs=3, gripper_type='85') -> None:
        ...
        # define environment
        self.physicsClient = connect(use_gui=True)
        ...
        # p.changeDynamics(self.robotID, self.joints['left_inner_finger_pad_joint'].id, lateralFriction=100, spinningFriction=10, rollingFriction=10)
        p.changeDynamics(self.robotID, self.joints['left_inner_finger_pad_joint'].id, lateralFriction=10.0, spinningFriction=1.0, rollingFriction=1.0, frictionAnchor=True)
        p.changeDynamics(self.robotID, self.joints['right_inner_finger_pad_joint'].id, lateralFriction=10.0, spinningFriction=1.0, rollingFriction=1.0, frictionAnchor=True)
    ...
    def load_robot(self, model_path, pose, orne):
        self.robotGripperID = load_model(model_path, (pose, orne), fixed_base=True)
        
        return self.robotGripperID
    ...
    def move_joints_to_target(self, robotID, jointPose, numJoints):
    ...
    def step(self, position: tuple, angle: float, action_type: str, camera: Camera, debug: bool = False):
        """
        ...
        for _ in range(max_step):
            ...
            for i, name in enumerate(self.controlJoints[:-1]):  # Filter out the gripper
                ...
                p.setJointMotorControl2(self.robotID, joint.id, p.POSITION_CONTROL,
                                        ...
                                        maxVelocity=joint.maxVelocity if custom_velocity is None else custom_velocity)
            # for _ in range(480):
            self.step_simulation()
