...
class GoogleModels(Models):
    ...
    def load_objects(self):
        ...
        for filename in self.obj_files:
            objectid = load_model(filename, ([0.0, -0.8, 0.9], [0, 0, 0, 1]))
            objectidlist.append(objectid)

        return objectidlist

    def load_single_objects(self, filename):
        objectid = load_model(filename, ([0, 0, 0], [0, 0, 0, 1]))
